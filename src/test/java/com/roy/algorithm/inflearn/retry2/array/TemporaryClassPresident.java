...
// 그는 자기반 학생 중에서 1학년부터 5학년까지 지내오면서 한번이라도 같은 반이었던 사람이 가장 많은 학생을 임시 반장으로 정하려한다.
...
// - 입력설명
// 첫째 줄에는 반의 학생 수를 나타내는 정수가 주어진다. 학생 수는 3 이상 1000이하이다. 둘째 줄부터는 1번 학생부터
// 차례대로 각 줄마다 1학년부터 5학년까지 몇 반에 속했었는지를 나타내는 5개의 정수가 빈칸 하나를 두고 주어진다.
...
public class TemporaryClassPresident {
    ...
    public int solution1(int[][] students) {
        ...
        for (int i = 0; i < studentCount; i++) {
            int tempFriendship = Integer.MIN_VALUE;
            ...
            if (tempFriendship > maxFriendship) {
                presidentNumber = i + 1;
                maxFriendship = tempFriendship;
            }
        }
